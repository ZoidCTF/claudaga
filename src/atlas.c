...
{
    SpritePose pose = { 0, SDL_FLIP_NONE };

    const Sprite *s = atlas_get(id);
    if (s->rot_frames < 2) return pose;

    /* Fold the heading into [0, 360). */
    float h = fmodf(heading_deg, 360.0f);
    if (h < 0.0f) h += 360.0f;

    /* The stored quadrant runs from west (270) round to north (360). Each
       mirroring maps some other quadrant onto it:
         none  leaves a heading alone,
         horizontal sends h to 360-h,
         vertical   sends h to 180-h,
         both       sends h to h+180.
       Pick the mirroring whose inverse lands the target inside 270..360. */
    float base;
    if (h >= 270.0f) {                       /* west  -> north */
        base = h;
        pose.flip = SDL_FLIP_NONE;
    } else if (h <= 90.0f) {                 /* north -> east  */
        base = 360.0f - h;
        pose.flip = SDL_FLIP_HORIZONTAL;
    } else if (h <= 180.0f) {                /* east  -> south */
        base = h + 180.0f;
        pose.flip = (SDL_RendererFlip)(SDL_FLIP_HORIZONTAL | SDL_FLIP_VERTICAL);
    } else {                                 /* south -> west  */
        base = 540.0f - h;
        pose.flip = SDL_FLIP_VERTICAL;
    }

    /* Frame 0 sits at west, the last rotation frame at north. */
    int   last = s->rot_frames - 1;
    float step = 90.0f / (float)last;
    int   f    = (int)((base - 270.0f) / step + 0.5f);
    if (f < 0)    f = 0;
    if (f > last) f = last;
...
{
    const Sprite *s = atlas_get(id);
    int extra = s->count - s->rot_frames;   /* the alternate forward poses */
    if (extra <= 0) return s->rot_frames > 0 ? s->rot_frames - 1 : 0;

    /* Frame 6 (last rotation) and frame 7 (the spare) are both north-facing,
       so alternating them reads as a wing beat. */
    return (phase & 1) ? s->rot_frames : s->rot_frames - 1;
}

MissileDir atlas_missile_dir(float vx, float vy)
{
    /* Screen y grows downward, so negate it to get a conventional angle, then
       measure clockwise from north to match the MissileDir order. */
    double a = atan2((double)vx, (double)-vy);          /* -PI..PI, 0 = north */
    if (a < 0) a += 2.0 * M_PI;
    int step = (int)floor(a / (M_PI / 4.0) + 0.5);      /* nearest 45 degrees */
    return (MissileDir)(step % MISSILE_DIR_COUNT);
}
